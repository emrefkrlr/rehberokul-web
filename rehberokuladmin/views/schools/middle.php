        ...
        <table class="table table-bordered table-striped mb-none" id="datatable-tabletools-kindergarten-ks">
            ...
            <?php foreach ($viewModel as $schools) :?>
                <tr>
                    ...
                    <td class="center">
                        <a href="<?php echo strtolower(get_class($this));?>/viewEditMiddle/<?php echo $schools['link'];?>" title="Düzenle"><button type="button" class="mb-xs mt-xs mr-xs btn btn-xs btn-info" <?php echo $rm->roleAuths($_SESSION['user_data']['role'], Controller::$active_id)['editing'] == 0 || $schools['admin_approved'] == 0 ? "style='display: none;'" : "";?>><i class="fa fa-edit"></i></button></a>
                        ...
                        <a href="<?php echo strtolower(get_class($this));?>/viewDeleteMiddle/<?php echo $schools['link'];?>" title="Sil"><button type="button" class="mb-xs mt-xs mr-xs btn btn-xs btn-danger" <?php echo $rm->roleAuths($_SESSION['user_data']['role'], Controller::$active_id)['deleting'] == 0 || $schools['admin_approved'] == 0 ? "style='display: none;'" : "";?>><i class="fa fa-trash"></i></button></a>
                    </td>
                </tr>
            <?php endforeach;?>
