    ...
    try {
        ...
        foreach($schools as $school) {
            foreach($sssler as $sss) {
                ...
                if($sss['sss_connection'] == 'okul-saatleri') {
                    if ($sss['sss_answer_type'] == 'otomatik') {
                        $db->statement = $db->databaseHandler->prepare("SELECT monday_start, monday_end FROM `school_hours` where school_id =" . $school['id']);
                        $db->statement->execute();
                        $fetchDataHours = $db->statement->fetch();
