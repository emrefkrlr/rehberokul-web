...
function saveSSS() {
    try {
        ...
        foreach($schools as $school) {
            $danismanVarmi = $school['counselor'] == 1 ? true : false;
            foreach($sssler as $sss) {
                if($sss['sss_connection'] == 'danisman' || $sss['sss_connection'] == 24) { // Rehberlik Tanışman
                    $db->findByColumn('school_facility', 'school_id', $school['id'], null, array('facility_id'));
                    $db->addAndClause('facility_id', 24);
                    $school_facility = $db->single();
                    if($danismanVarmi || $school_facility) {
