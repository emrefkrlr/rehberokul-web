    ...
    try {
        ...
        foreach($schools as $school) {
            foreach($sssler as $sss) {
                ...
                if($sss['sss_connection'] == 'servisler') { // Servisler İmkanlar için
                    if ($sss['sss_answer_type'] == 'otomatik') {
                        $db->statement = $db->databaseHandler->prepare("SELECT name FROM `facility` WHERE type=2 and id IN (select facility_id from school_facility where school_id = " . $school['id'] . ")");
