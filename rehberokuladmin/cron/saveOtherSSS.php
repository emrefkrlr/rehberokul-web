...
function saveSSS() {
    try {
        ...
        foreach($schools as $school) {
            ...
            if($sss['sss_connection'] != 'fiziksel-imkanlar' && $sss['sss_connection'] != 'yabanci-diller'
            && $sss['sss_connection'] != 'servisler' && $sss['sss_connection'] != 'aktiviteler' &&
                $sss['sss_connection'] != 'yas-araligi' && $sss['sss_connection'] != 'danisman' &&
                $sss['sss_connection'] != 24 && $sss['sss_connection'] != 'servis' &&
                $sss['sss_connection'] != 26 && $sss['sss_connection'] != 'bagimsiz' &&
                $sss['sss_connection'] != 'okul-saatleri') {
                    if ($sss['sss_answer_type'] == 'otomatik') {
                        $db->statement = $db->databaseHandler->prepare("SELECT name FROM `facility` where id IN (select facility_id from school_facility where facility_id=" . $sss['sss_connection'] . " AND school_id = " . $school['id'] . ")");
