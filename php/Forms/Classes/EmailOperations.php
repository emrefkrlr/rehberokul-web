...
class EmailOperations {
    ...
    public function createToken($inputValue) {
        $returnValue = sha1($this->permalink($inputValue).uniqid());     
        return $returnValue;
    }

    public function convertPermalink($inputValue) {
        $returnValue = $this->permalink($inputValue);     
        return $returnValue;
    }

    public function convertTextToTitleCase($inputValue) {
        $returnValue = $this->ucwords_tr($inputValue);     
        return $returnValue;
    }

    public function securePassword($inputValue) {
        $hashedPassword = hash("sha384", $inputValue);
        return $hashedPassword;
    }

    public function spamSecurity(array $inputValues) {
        $isSecure = 1;
        foreach ($inputValues as $inputValue) {
            if (strlen($inputValue) < 2) {
                $isSecure = 0;
            } else {
                $spamFilterKeywords = array("offer", "secure", "adult", "girl", "web", "$", "sex", "date", "free", "buy", "call", "click", "order", "Offer", "Secure", "Adult", "Girl", "Web", "Sex", "Date", "Free", "Buy", "Call", "Click", "Order");
                foreach ($spamFilterKeywords as $spamFilterKeyword) {
                    if (strpos($inputValue, $spamFilterKeyword) !== false) {
                        $isSecure = 0;
                        break;
                    }
                }
