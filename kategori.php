...
$pageTitle = "Rehber Kategoriler | Rehber Okul";
$pageDescription = "Size faydalı olacak tüm yazılarımızı kategoriler altında görüntüleyerek inceleyebilirsiniz";
...
if (isset($_SERVER['REQUEST_URI'])) {
    ...
    if (count($url) > 1) {
        // kategori var
        $category = $url[2];
        $db->where('link', $category);
        $getCategoryName = ucwords($db->getValue('tag', 'name'));

        $getBlogsByCategory = $db->query("SELECT * FROM blog WHERE id IN (
                                                SELECT blog_id FROM blog_tags where tag_id = (
                                                SELECT id FROM tag where link = '".$category . "'))
        ...
        $postByPage = array_slice($getBlogsByCategory, $startPoint, $pageLimit);

    }else {
        header("location", "https://www.rehberokul.com/sayfa-bulunamadi");
    }
}
?>
	...
	<section class="p-about com-padd">
        ...
        <div class="row">
            <ul class="pagination list-pagenat">
                <?php
                if (count($getBlogsByCategory) > 0) {
                    $totalPage = ceil(count($getBlogsByCategory) / $pageLimit);
                    if ($totalPage != 1) {
                        if ($pageNumber != 1) {
                            echo '<li class="waves-effect"><a class="page-link" href="'.WEBURL.'kategori/'.$category.'/'.($pageNumber-1).'"><i class="fa fa-angle-left"></i></a></li>';
                        }
                        if ($pageNumber == 1 || $pageNumber == 2 || $pageNumber == 3) {
                            for ($i=1; $i <= 7; $i++) {
                                if ($i <= $totalPage) {
                                    ...
                                    echo '"><a class="page-link" href="'.WEBURL.'kategori/'.$category.'/'.$i.'">'.$i.'</a></li>';
                                }
                            }
                        ...
                        if ($pageNumber != $totalPage) {
                            echo '<li class="waves-effect"><a class="page-link" href="'.WEBURL.'kategori/'.$category.'/'.($pageNumber+1).'"><i class="fa fa-angle-right"></i></a></li>';
                        }
                    }
                }
                ?>
